...
import { Search, ShoppingBag, Plus, Minus, X, Printer, Loader2, UserPlus, Check, PackageSearch, User, Phone, MapPin, Mail, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function POSPage() {
  ...
  const actualAmountPaid = amountPaidInput === '' ? 0 : Number(amountPaidInput);
  const balanceDue = Math.max(0, cart.total - actualAmountPaid);
  const cartItemCount = cart.items.reduce((sum, item) => sum + item.quantity, 0);

  const orderStatus: 'Pending' | 'Partial' | 'Paid' =
    actualAmountPaid <= 0 ? 'Pending' : actualAmountPaid < cart.total ? 'Partial' : 'Paid';
  ...
  }, [cart.discountAmount, categoriesInCart]);
      ...
      try {
        ...
        try {
          ...
          if (localCats.length > 0) {
            ...
            toast.success('Loaded from offline storage');
          } else {
  ...
  const handleAddDealToCart = (deal: any) => {
    ...
    toast.success(`${deal.name} added`, { id: `add-${deal._id}` });
  };
  ...
  const handlePlaceOrder = async () => {
    ...
    if (!isWalkIn && !customerName.trim()) {
      toast.error('Please enter a customer name or check "Walk-in Order"');
      return;
    }
    ...
    if (cart.discountAmount > 0) {
      ...
      if (validCategories.length === 0) {
        toast.error('Select at least one category for the discount');
        return;
      }
    }
    ...
    const orderPayload = {
      ...
      cashier: '60d0fe4f5311236168a109ca',
    ...
    try {
      ...
      if (error.code === 'ECONNABORTED' || !error.response) {
        ...
        toast.success(`Order saved offline (${tempId})`, { duration: 5000 });
        dispatch(clearCart());
        handleClearCustomer();
        setAmountPaidInput('');
        setSelectedDiscountCategories([]);
  ...
  const ReceiptContent = ({ copyLabel }: { copyLabel: string }) => (
    <div className="max-w-md mx-auto" style={{ padding: '20px' }}>
      ...
      <div className="text-center text-[11px] text-gray-500 mt-6 pt-3 border-t border-gray-200">
        Software by: Rana Zavyar 03045087177
      </div>
    </div>
  );
    ...
    <>
      ...
      <div className="h-full flex flex-col lg:flex-row gap-6 print:hidden relative">
            ...
            {loadingData ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-4">
                ...
                {activeCategory !== 'Deals' && filteredItems.length === 0 && (
                  <div className="col-span-full flex flex-col items-center justify-center text-muted-foreground py-12 gap-2">
                    <PackageSearch size={36} className="opacity-40" />
                    <p>{searchQuery ? `No items match "${searchQuery}"` : 'No menu items found'}</p>
                  </div>
                )}
              </div>
            )}
        ...
        <div className={`
          ...
          lg:relative lg:inset-auto lg:w-[420px] lg:translate-y-0 lg:flex lg:h-full lg:bg-card lg:rounded-xl lg:border lg:border-border lg:overflow-hidden
        ...
        `}>
          ...
          <div className="p-4 border-b border-border bg-secondary/50 flex justify-between items-center">
            ...
            <div className="flex items-center gap-4">
              {cart.items.length > 0 && (
                <button
                  onClick={() => { if (confirm('Clear cart?')) dispatch(clearCart()); }}
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4">
            {cart.items.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-muted-foreground space-y-3 opacity-60">
                <ShoppingBag size={48} />
                <p>Cart is empty</p>
                <p className="text-xs">Tap any item to add it</p>
              </div>
            ) : (
              <>
                {cart.items.map(item => (
          ...
          <div className="p-4 border-t border-border bg-secondary/30 space-y-3">
            ...
            <button
              ...
              className="w-full flex items-center justify-between text-sm hover:bg-secondary/50 p-1 rounded transition-colors"
            >
              <div className="flex items-center gap-2">
                ...
                {!customerName && !isWalkIn && (
                  <span className="text-destructive text-[10px] font-medium">*Required</span>
                )}
                {isWalkIn && (
                  <span className="text-muted-foreground text-[10px]">(Walk-in)</span>
                )}
              </div>
              <div className="flex items-center gap-3">
                {/* Walk-in Checkbox - Now showing next to Required text */}
                {!customerName && !isWalkIn && (
                  <label className="flex items-center gap-1.5 text-[10px] cursor-pointer text-muted-foreground hover:text-foreground transition-colors">
                    <input
                      type="checkbox"
                      checked={isWalkIn}
                      onChange={(e) => {
            ...
            {showCustomerSection && (
              <div className="bg-background p-3 rounded-lg border border-border space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  {!isWalkIn && !customerName && (
                    <span className="text-destructive text-[10px] uppercase tracking-wider">Required</span>
                  )}
                </div>
                ...
                {!isWalkIn && (
                  <div className="space-y-2">
                    ...
                    <div className="relative" ref={customerSearchRef}>
                      <div className="flex gap-2">
                        ...
                        {/* <button
                          ...
                          className="shrink-0 px-3 py-2 bg-primary text-primary-foreground rounded text-sm font-medium flex items-center gap-1.5 hover:bg-primary/90 transition-colors"
                        >
                      ...
                      <div className="flex items-center justify-between bg-primary/5 border border-primary/20 rounded-lg px-3 py-2">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          {selectedCustomer.phone && (
                            <span className="text-xs text-muted-foreground truncate">· {selectedCustomer.phone}</span>
                          )}
                        </div>
                        <button
                          type="button"
                    ...
                    {/* Manual Customer Input (shown when no customer selected) */}
                    {!selectedCustomer && (
                      <div className="space-y-2">
                        <input
                          type="text"
                          ...
                          className={`w-full px-3 py-2 bg-secondary border rounded text-sm focus:outline-none focus:ring-1 focus:ring-primary ${
                            !isWalkIn && !customerName.trim() ? 'border-destructive/50 ring-1 ring-destructive/50' : 'border-border'
                          }`}
                        />
            ...
            >
              <span className="text-muted-foreground flex items-center gap-1">
                ...
                {cart.discountAmount > 0 && (
                  <span className="text-destructive font-medium">(-Rs. {cart.discountAmount})</span>
                )}
              </span>
              <div className="flex items-center gap-2">
                {cart.discountAmount > 0 && (
                ...
                {showDiscountSection ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
              </div>
            </button>
              ...
              <div className="bg-background p-3 rounded-lg border border-border space-y-2">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 bg-secondary border border-border rounded text-sm focus:outline-none focus:ring-1 focus:ring-primary"
                  />
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="px-3 py-2 bg-secondary text-foreground rounded text-sm hover:bg-secondary/80"
                  >
                    Clear
                  </button>
                </div>

                {cart.discountAmount > 0 && categoriesInCart.length > 0 && (
                  <div className="space-y-1">
                    <p className="text-xs font-medium text-muted-foreground">Apply to:</p>
                    <div className="flex flex-wrap gap-2">
                      {categoriesInCart.map(cat => (
                        <label key={cat} className="flex items-center gap-1.5 text-xs cursor-pointer bg-secondary/50 px-2 py-1 rounded">
                          <input
                            type="checkbox"
                            checked={selectedDiscountCategories.includes(cat)}
                            onChange={(e) => {
                              if (e.target.checked) {
                                setSelectedDiscountCategories([...selectedDiscountCategories, cat]);
                              } else {
                                setSelectedDiscountCategories(selectedDiscountCategories.filter(c => c !== cat));
                              }
                            }}
                            className="rounded border-border text-primary focus:ring-primary"
                          />
            ...
            <div className="flex justify-between items-center text-sm pt-2">
              ...
              <div className="flex items-center gap-1.5">
                {actualAmountPaid !== cart.total && cart.total > 0 && (
                  <button
                    type="button"
                    onClick={() => setAmountPaidInput(cart.total.toString())}
                ...
                {actualAmountPaid !== 0 && (
                  <button
                    ...
                    className="text-[11px] font-medium text-muted-foreground hover:underline whitespace-nowrap"
                  >
                    Reset
                  </button>
                )}
