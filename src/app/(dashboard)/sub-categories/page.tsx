...
export default function SubCategoriesPage() {
  ...
  const [newCatMainCategory, setNewCatMainCategory] = useState('');
  ...
  const fetchCategories = async () => {
    try {
      const [res, mainRes] = await Promise.all([
        axios.get('/api/sub-categories'),
        axios.get('/api/categories')
      ]);
      setCategories(res.data);
      setMainCategories(mainRes.data);
      if (mainRes.data.length > 0 && !newCatMainCategory) {
        setNewCatMainCategory(mainRes.data[0]._id);
      }
    } catch (error) {
  ...
  const handleAddCategory = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCatId) {
        await axios.patch(`/api/sub-categories/${editingCatId}`, {
          name: newCatName,
      ...
      setNewCatName('');
      setNewCatMainCategory(mainCategories.length > 0 ? mainCategories[0]._id : '');
      setNewCatIcon('🏷️');
  ...
  const handleEditClick = (category: any) => {
    ...
    setNewCatMainCategory(category.mainCategory?._id || '');
      ...
      {showAddForm && (
        <div className="bg-card border border-border rounded-xl shadow-md p-6 mb-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold">{editingCatId ? 'Edit Sub Category' : 'New Sub Category'}</h3>
            <button onClick={() => {
              setShowAddForm(false);
              setEditingCatId(null);
              setNewCatName('');
          ...
          <form onSubmit={handleAddCategory} className="flex flex-col gap-4">
            ...
            <div className="mt-2 flex justify-end">
              ...
              >
                {isSubmitting ? <Loader2 className="animate-spin" size={18} /> : 'Save Sub Category'}
              </button>
            </div>
          </form>
      </div>
